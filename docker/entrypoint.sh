...
#
# Se ejecuta al arrancar el contenedor. Hace tres cosas:
#   1. Sourcea ROS 2 y el workspace de CycloneDDS
#   2. Configura DDS segun GO2_MODE (sim -> domain 1 por lo; real -> domain 0
#      por la interfaz Ethernet)
#   3. Arranca el escritorio virtual y noVNC si GO2_GUI=novnc
#
# Sustituye a los alias go2sim / go2real del .bashrc, que no existen aqui.
...
if [[ "${GO2_GUI}" == "novnc" ]]; then
    export DISPLAY=":99"
    # OpenGL por software: no hay GPU garantizada en la maquina anfitriona.
    ...
    echo "escritorio virtual listo -> abre http://localhost:6080/vnc.html"
